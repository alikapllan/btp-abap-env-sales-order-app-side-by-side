"! @testing BDEF:ZRAP100_R_TravelTP_AHK
CLASS zrap100_tc_travel_eml_ahk DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA begin_date           TYPE /dmo/begin_date.
    CLASS-DATA end_date             TYPE /dmo/end_date.
    CLASS-DATA agency_mock_data     TYPE STANDARD TABLE OF /dmo/agency.
    CLASS-DATA customer_mock_data   TYPE STANDARD TABLE OF /dmo/customer.
    CLASS-DATA carrier_mock_data    TYPE STANDARD TABLE OF /dmo/carrier.
    CLASS-DATA flight_mock_data     TYPE STANDARD TABLE OF /dmo/flight.

    CLASS-METHODS class_setup.    " setup test double framework
    CLASS-METHODS class_teardown. " stop test doubles

    METHODS setup.          " reset test doubles
    METHODS teardown.       " rollback any changes

    " CUT: create with action call and commit
    METHODS create_with_action FOR TESTING RAISING cx_static_check.

ENDCLASS.


CLASS zrap100_tc_travel_eml_ahk IMPLEMENTATION.
  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                               i_for_entities = VALUE #( ( i_for_entity = 'ZRAP100_R_TravelTP_AHK' ) ) ).
    ...
    flight_mock_data   = VALUE #( ( carrier_id    = '123'
                                    connection_id = '9876'
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_AHK
           ENTITY Travel
    ...
    cl_abap_unit_assert=>assert_initial( msg = 'failed'
                                         act = failed ).
    cl_abap_unit_assert=>assert_initial( msg = 'reported'
                                         act = reported ).

    " expect a newly created record in mapped tables
    cl_abap_unit_assert=>assert_not_initial( msg = 'mapped-travel'
                                             act = mapped-travel ).
    ...
    cl_abap_unit_assert=>assert_initial( msg = 'commit_failed'
                                         act = commit_failed ).
    cl_abap_unit_assert=>assert_initial( msg = 'commit_reported'
                                         act = commit_reported ).

    " read the data from the persisted travel entity (using the test doubles)
    SELECT * FROM ZRAP100_R_TravelTP_AHK INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
    " assert the existence of the persisted travel entity
    cl_abap_unit_assert=>assert_not_initial( msg = 'travel from db'
                                             act = lt_travel ).
    " assert the generation of a travel ID (key) at creation
    cl_abap_unit_assert=>assert_not_initial( msg = 'travel-id'
                                             act = lt_travel[ 1 ]-TravelID ).
